---
title: ShapeRange.Glow Property (Excel)
keywords: vbaxl10.chm640144
f1_keywords:
- vbaxl10.chm640144
ms.prod: EXCEL
api_name:
- Excel.ShapeRange.Glow
ms.assetid: d2b7670f-2715-51f6-1c67-434e3ab39a67
---


# ShapeRange.Glow Property (Excel)

Returns a  **[GlowFormat](http://msdn.microsoft.com/library/glowformat-object-office%28Office.15%29.aspx)** object for a specified shape range that contains glow formatting properties for the shape range. Read-only.


## Syntax

 _expression_ . **Glow**

 _expression_ A variable that represents a **ShapeRange** object.


## Remarks

The glow effect adds a vibrant colored edge to graphics.


## See also


#### Concepts


[ShapeRange Object](shaperange-object-excel.md)

