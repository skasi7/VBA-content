...
Because  [CommandBar](http://msdn.microsoft.com/library/commandbar-object-office%28Office.15%29.aspx) objects have been deprecated since Outlook, shortcut menu events of the [Application](application-object-outlook.md) object that relied on the **CommandBar** object are being deprecated as well, and might not work as expected. These events include the following:
...
-  **ViewContextMenuDisplay** event
 ...
 To customize shortcut menus, implement the **[IRibbonExtensibility](http://msdn.microsoft.com/library/iribbonextensibility-object-office%28Office.15%29.aspx)** interface in your add-in. Specifically, implement the **[GetCustomUI](http://msdn.microsoft.com/library/iribbonextensibility-getcustomui-method-office%28Office.15%29.aspx)** method of the **IRibbonExtensibility** interface so that when Office calls the **GetCustomUI** method and specifies **Microsoft.Outlook.Explorer** as the ribbon ID, the method loads the custom shortcut menu that is delimited by the `contextMenus` tag in XML. For a complete listing of ribbon identifiers, see [Implementing the IRibbonExtensibility Interface](implementing-the-iribbonextensibility-interface.md).
